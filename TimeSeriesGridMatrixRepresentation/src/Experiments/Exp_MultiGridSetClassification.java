 ...
 */
public class Exp_MultiGridSetClassification {
    public static void run() throws IOException
    {
        ...
        for (File file : files) {
            ...
            {
                Grid grid = new Grid();
                SetTS[] trainSets = grid.dataset2Set(trainOrg);
                thresholdList.add(Validation.LOOCV_Set(trainSets));
            }
            ...
            for (int g_idx=0; g_idx<m; g_idx++)
            {
                Grid grid = new Grid();
                SetTS[] trainSets = grid.dataset2Set(trainOrg);
                double trainErrorRate = Validation.LOOCV_Set(trainSets);
                if(trainErrorRate > threshold)
                {
                    g_idx--;
                    continue;
                }
                grid.setTrainSets(trainSets);
                grids[g_idx] = grid ;
            }

            //conduct 1-NN classification and get error rate by Multi Grid STS3
            int predictCount = 0;
            int errorCount = 0;
            for(int q_idx=0; q_idx<testOrg.size(); q_idx++)
            {
                ...
                for(int g_idx=0; g_idx<m; g_idx++)
                {
                    ...
                    SetTS querySet = grid.ts2Set(testOrg.get(q_idx));

                    ArrayList<Integer> predictLabel = Validation.searchListByJaccard(grid.getTrainSets(), querySet);
